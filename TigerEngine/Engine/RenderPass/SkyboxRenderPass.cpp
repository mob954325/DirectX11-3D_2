...
void SkyboxRenderPass::Init(ComPtr<ID3D11Device>& device, ComPtr<ID3D11DeviceContext>& deviceContext, UINT width, UINT height)
{
    clientWidth = width;
    clientHeight = height;
    CreateCube(device);
...
void SkyboxRenderPass::Execute(ComPtr<ID3D11DeviceContext> &context, std::shared_ptr<Scene> scene, std::shared_ptr<Camera> cam)
{
    // 카메라용 뷰 행렬과, 투영행렬
...
void SkyboxRenderPass::SetDepthStencilView(ComPtr<ID3D11DepthStencilView> &dsv)
{
    depthStencliView = dsv;
}

void SkyboxRenderPass::SetRenderTargetView(ComPtr<ID3D11RenderTargetView> &rtv)
{
    renderTargetView = rtv;
}

void SkyboxRenderPass::CreateCube(ComPtr<ID3D11Device>& device)
{
