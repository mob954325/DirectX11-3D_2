...
{
    auto mappedInfo = boneMappingTable.find(boneName);
    if (mappedInfo == boneMappingTable.end()) 
        throw std::out_of_range("Bone name not Found " + boneName);

    return bones[mappedInfo->second];
}
...
{
	auto mappedInfo = bonesOffset.find(boneName);
	if (mappedInfo == bonesOffset.end())
...
bool SkeletonInfo::CreateFromAiScene(const aiScene* pAiScene)
{
	...
	for (int i = 0; i < meshNum; i++)
	{
		...
		for (int j = 0; j < boneNum; j++)
		{
			...
			bonesOffset.insert({ name, offsetMat });
		}
	}
...
void SkeletonInfo::CreateBoneInfoFromNode(const aiNode* pAiNode)
{
	...
	bones.push_back(currInfo);
	boneMappingTable.insert({ currentBoneName, static_cast<int>(bones.size() - 1) });

	int currentBoneIndex = static_cast<int>(bones.size());
