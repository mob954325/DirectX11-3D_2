...
void FBXResourceManager::ProcessNode(std::shared_ptr<FBXResourceAsset>& asset, aiNode* pNode, const aiScene* pScene)
{
	string boneName = pNode->mName.C_Str();
	BoneInfo boneInfo = asset->skeletalInfo.GetBoneInfoByName(boneName);
	int boneIndex = asset->skeletalInfo.GetBoneIndexByName(boneName);

	// node 추적
	for (UINT i = 0; i < pNode->mNumMeshes; i++)
	{
		aiMesh* mesh = pScene->mMeshes[pNode->mMeshes[i]];
		asset->meshes.push_back(this->ProcessMesh(asset, mesh, pScene));
		asset->meshes.back().refBoneIndex = boneIndex;

		asset->meshes.back().SetMaterial(pScene->mMaterials[mesh->mMaterialIndex]);

		// weight 저장
		ProcessBoneWeight(asset, mesh);
	}

	for (UINT i = 0; i < pNode->mNumChildren; i++)
	{
		this->ProcessNode(asset, pNode->mChildren[i], pScene);
	}
}
...
std::shared_ptr<FBXResourceAsset> FBXResourceManager::LoadFBXByPath(ComPtr<ID3D11Device>& pDevice, ComPtr<ID3D11DeviceContext>& pDeviceContext, std::string path)
{
	...
	// bone offest 버퍼 채우기
	for (auto& bone : sharedAsset->skeletalInfo.bones)
	{
		...
		sharedAsset->boneOffsets.boneOffset[boneIndex] = offsetMat;
	}
